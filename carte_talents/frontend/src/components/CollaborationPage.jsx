...
import { Users2, Search, MapPin, Briefcase, Send, X, Check, Sparkles, AlertCircle, Loader2, LogIn, Lock } from 'lucide-react';
...
export default function CollaborationPage({ talents, setView, setCurrentProfile, showToast }) {
  ...
  // Filtrer pour exclure son propre profil
  const filtered = talents.filter(t => {
    ...
    const notSelf = !user || t.user_id !== user.id; // Ne pas afficher son propre profil
    return matchSearch && matchSkill && notSelf;
  });

  const contact = (t) => {
    if (!isAuthenticated) {
      showToast?.('Vous devez être connecté pour envoyer une demande', 'error');
      setView('auth');
      return;
    }
  ...
  // Si l'utilisateur n'est pas connecté, afficher un message de blocage
  if (!isAuthenticated) {
    return (
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6">
          ...
          >
            ...
            <p className="text-dark-400 mb-8 text-lg">
              Pour envoyer des demandes de collaboration et contacter d'autres talents, 
            ...
            <div className="mt-10 pt-8 border-t border-white/10">
              <p className="text-dark-500 text-sm mb-4">Pourquoi se connecter ?</p>
              <div className="grid sm:grid-cols-3 gap-4 text-left">
                <div className="p-4 rounded-xl bg-dark-800/50">
                  <Send className="w-6 h-6 text-emerald-400 mb-2" />
                  <p className="text-white text-sm font-medium">Envoyer des demandes</p>
                  <p className="text-dark-500 text-xs mt-1">Contactez les talents qui vous intéressent</p>
                </div>
                <div className="p-4 rounded-xl bg-dark-800/50">
                  <Users2 className="w-6 h-6 text-primary-400 mb-2" />
                  <p className="text-white text-sm font-medium">Recevoir des offres</p>
                  <p className="text-dark-500 text-xs mt-1">Soyez contacté par d'autres professionnels</p>
                </div>
                <div className="p-4 rounded-xl bg-dark-800/50">
                  <Check className="w-6 h-6 text-accent-400 mb-2" />
                  <p className="text-white text-sm font-medium">Accepter ou refuser</p>
                  <p className="text-dark-500 text-xs mt-1">Gérez vos collaborations</p>
                </div>
              </div>
            </div>
          </motion.div>
    ...
    <div className="min-h-screen bg-dark-950 pt-24 pb-12 relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-dark-400">Connecté en tant que <span className="text-primary-400">{user?.name}</span></p>
        </div>
        ...
        <div className="glass p-6 mb-10">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Rechercher par nom ou compétence..." value={search} onChange={e => setSearch(e.target.value)} className="input-field pl-12" />
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              {filtered.length === 0 ? (
                <div className="glass p-8 text-center">
                  <Users2 className="w-12 h-12 text-dark-600 mx-auto mb-4" />
                  <p className="text-dark-400">Aucun talent trouvé avec ces critères.</p>
                </div>
              ) : (
                filtered.slice(0, 10).map(t => (
                  <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass p-5 hover:bg-white/5 transition-colors">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-dark-400 text-sm mb-3 line-clamp-2">{t.bio}</p>
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {(t.skills || []).slice(0, 5).map((s, i) => <span key={i} className="tag text-xs">{s}</span>)}
                        </div>
                      ...
                      <div className="flex flex-col gap-2">
                        <button onClick={() => { setCurrentProfile(t); setView('profile'); }} className="px-4 py-2 rounded-xl text-sm bg-dark-800 text-dark-300 hover:bg-dark-700 transition-colors">Profil</button>
                        <button 
                          onClick={() => contact(t)} 
                          disabled={!t.user_id}
                          className={`px-4 py-2 rounded-xl text-sm flex items-center gap-2 ${
                            t.user_id 
                              ? 'bg-gradient-to-r from-primary-600 to-accent-600 text-white hover:shadow-lg hover:shadow-primary-500/25 transition-all' 
                              : 'bg-dark-700 text-dark-500 cursor-not-allowed'
                          }`}
                          title={!t.user_id ? 'Ce profil n\'est pas lié à un compte' : 'Envoyer une demande de collaboration'}
                        >
          ...
          <div>
            ...
            <div className="space-y-4">
              {ideas.map((idea, i) => (
                <div key={i} onClick={() => setSkill(idea.skills[0])} className="glass p-5 cursor-pointer hover:bg-white/5 transition-colors">
            ...
            {/* Raccourci vers l'inbox */}
            <div className="mt-6 glass p-5 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setView('inbox')}>
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-primary-500/10">
                  <Send className="w-5 h-5 text-primary-400" />
                </div>
                <div>
                  <h3 className="font-bold text-white">Mes demandes</h3>
                  <p className="text-dark-400 text-sm">Voir les demandes reçues et envoyées</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 glass p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2 rounded-xl bg-emerald-500/10"><Sparkles className="w-5 h-5 text-emerald-400" /></div>
                <h3 className="font-bold text-white">Astuce</h3>
              </div>
              <p className="text-dark-400 text-sm">Cliquez sur une idée de projet pour filtrer les talents par compétence correspondante.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showModal && target && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dark-950/80 backdrop-blur-sm" onClick={() => !sent && !sending && setShowModal(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="glass p-8 max-w-lg w-full" onClick={e => e.stopPropagation()}>
              {!sent ? (
                <>
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary-500 to-accent-600 flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">
                      {target.name?.charAt(0)}
                    </div>
                    <div className="flex-1">
                      <h2 className="text-2xl font-bold text-white">Contacter {target.name}</h2>
                      <p className="text-dark-400 text-sm">{target.location}</p>
                    </div>
                  ...
                  <p className="text-dark-400 mb-4">Décrivez votre projet ou proposition de collaboration. {target.name} recevra votre message et pourra l'accepter ou le refuser.</p>
                  ...
                  <textarea 
                    ...
                    placeholder="Bonjour, je travaille sur un projet intéressant et je pense que vos compétences seraient parfaites pour notre équipe..." 
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-dark-500 text-sm mt-2">Vous recevrez une notification quand {target.name} répondra.</p>
                </div>
              )}
            </motion.div>
          </motion.div>
